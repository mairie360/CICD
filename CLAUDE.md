...
| `back-lib-cicd.yml` | Rust library | `cargo audit` + `cargo deny check advisories licenses`, `cargo lint_check` (a cargo alias the repo must define), Codecov upload, `cargo publish` to crates.io, commits version bump to `main` — both only when `semantic-tag` published a release (`new_release_published == 'true'`), so a push to `main` with no `feat`/`fix`/`perf` commit is a green no-op instead of a `cargo publish` of `version = ""` |
