...
- **`openapi-package/generate.sh`** — not an action: the shared generator both OpenAPI publishers call, so an API package and a BFF package have the same shape. Takes `<spec.json> <out_dir> <npm_name> <version>` and emits **TypeScript only** (plus `package.json`), all derived from the one contract: `endpoints/` + `model/` (orval `axios`, unchanged paths, so existing consumers keep working), `zod/index.ts` (orval `zod`: formats, bounds, defaults — success responses only), `openapi.ts` (`openapi-typescript` types: every status, error and response header), `contract.ts` (the whole contract as a value, `export const contract = {…} as const`) and `index.ts`. orval and openapi-typescript are pinned (`8.33.0`, `7.10.1`) so output only changes when the contract does. A final guard fails the run if a non-`.ts` file lands in the package.
- **`publish-openapi-rust`** — `cargo open_api > openapi.json` → `generate.sh` → publishes `@<org>/<package_name>-openapi` to `npm.pkg.github.com`. It no longer uses the API repo's own `orval.config.js`; the shared config wins so every package matches.
- **`publish-openapi-typescript`** — same for TS BFFs, taking the BFF's exported contract through `spec_file`. It publishes for real (it used to be a stub that staged a directory and published nothing).
